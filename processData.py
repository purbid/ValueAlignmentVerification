...
def create_example_input(
    ...
    answer_tokens, 
    answer_token_labels, 
    answer_tokens_corrected_pred,
    answer_token_labels_corrected_pred,
    passage_tokens, 
    ...
    # question tokens
    tokens_corrected_pred = ["question:"] + question_tokens
    token_labels_corrected_pred = [IGNORE_TAG] + question_token_labels

    # add passage tokens
    for i, p_tokens in enumerate(passage_tokens):
        if i == 0:
            tokens_corrected_pred += ["context:"]
            token_labels_corrected_pred += [IGNORE_TAG]
        tokens_corrected_pred += p_tokens
        token_labels_corrected_pred += passage_token_labels[i]
        
    # answer tokens
    tokens_corrected_pred = tokens_corrected_pred + ['answer:'] + answer_tokens_corrected_pred
    token_labels_corrected_pred = token_labels_corrected_pred + [IGNORE_TAG] + answer_token_labels_corrected_pred
    
    return {
        "id": str(id), 
        "text": tokens, 
        "feedback_tags": token_labels,
        "text_corrected_pred": tokens_corrected_pred, 
        "feedback_tags_corrected_pred": token_labels_corrected_pred
    }
...
def construct_model_inputs(args, ids, labels, inputs, split):
    ...
    for id, label, input in zip(ids, labels, inputs):
        ...
        ans_tokens, ans_labels = get_token_labels(
            args,
            answer_tokens, 
            answer_token_is_sent_starts, 
            text, 
        ...
        # get (corrected-prediction) answer tokens
        doc_corrected_pred = nlp(input['feedback']['corrected-prediction'].strip())   # we only labeled feedback for the first prediction
        answer_tokens_corrected_pred = []
        answer_token_is_sent_starts_corrected_pred = []
        for s in doc_corrected_pred.sents:
            s_text = s.text
            answer_tokens_corrected_pred += s_text.split()
            if args.feedback_level == 'subsentence':
                answer_token_is_sent_starts_corrected_pred += get_subsentence_starts(s_text.split())
            else:
                answer_token_is_sent_starts_corrected_pred += [True] + (len(s_text.split()) - 1) * [False]
        text_corrected_pred = input['feedback']['corrected-prediction']
        if len(answer_token_is_sent_starts_corrected_pred) == 0:
            continue
        answer_token_is_sent_starts_corrected_pred[0] = True

        spans_corrected_pred = []
        if len(label['errors']) > 0:
            spans_corrected_pred = label['errors']
        
        ans_tokens_corrected_pred, ans_labels_corrected_pred = get_token_labels(
            args,
            answer_tokens_corrected_pred, 
            answer_token_is_sent_starts_corrected_pred, 
            text_corrected_pred, 
        ...
        input_json = create_example_input(
            ...
            ans_tokens, 
            ans_labels, 
            ans_tokens_corrected_pred,
            ans_labels_corrected_pred,
            passage_tokens, 
            passage_token_labels
        )
